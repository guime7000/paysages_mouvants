...
// The value to modify when testing heart sequence offset in seconds
const int offset_organ = 60;
...
int urn1_start[4] = {1, 180, 420, 1320 + offset_organ};  // Start time offsets in min since Show is ON.
int urn1_stop[4] = {120, 300, 480, 1500 + offset_organ}; // Stop time offsets in min since Show is ON.
byte urn1_index = 0;
...
// Urne 3: top/stop en minute : (5,10), (15,18), (25,30), (35,40)
int urn3_start[4] = {300, 900, 1500 + offset_organ, 2100 + offset_organ}; // Start time offsets in min since Show is ON.
int urn3_stop[4] = {600, 1080, 1800 + offset_organ, 2400 + offset_organ}; // Start time offsets in min since Show is ON.
byte urn3_index = 0;
...
int urn5_start[6] = {1320 + offset_organ, 1500 + offset_organ, 1800 + offset_organ, 2040 + offset_organ, 2160 + offset_organ, 3000 + offset_organ}; // Start time offsets in min since Show is ON.
...
void loop()
{
  ...
  if ((start_time + (2100 + offset_organ) <= now()) && (now() < start_time + (2160 + offset_organ)))
  {
  ...
  if ((start_time + urn5_start[0] <= now()) && (now() < start_time + urn5_start[1]))
  { // Fete Foraine
  ...
  if ((start_time + urn5_start[1] <= now()) && (now() < start_time + urn5_start[2]))
  { // Faible mouvement (2 lampes)
    if (landscape_now - elapsed >= landscape_delay)
    {
  ...
  if ((start_time + urn5_start[2] <= now()) && (now() < start_time + urn5_start[3]))
  { // Montée (4 lampes) + Vivre
    if (landscape_now - elapsed >= landscape_delay)
    {
  ...
  if ((start_time + urn5_start[3] <= now()) && (now() < start_time + urn5_start[4]))
  { // Virer lum droite 1 min puis UNE sec de blackout
    if (landscape_now - elapsed >= landscape_delay)
    {
  ...
  if ((start_time + urn5_start[4] <= now()) && (now() < start_time + urn5_start[4] + 1))
  { // 1 sec BLACKOUT
    for (int jj = 0; jj < sizeof(landscape_light) / sizeof(int); jj++)
    {
      analogWrite(landscape_light[jj], 0);
  ...
  if ((start_time + urn5_start[4] + 1 <= now()) && (now() < start_time + urn5_start[5]))
  { // CHENILLARD
